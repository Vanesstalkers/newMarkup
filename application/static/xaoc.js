...
var extend = function(target, varArgs) { // .length of function is 2
  'use strict';
...
function loadRes(src, onetime, callback, config = {})
{
	...
	if(config.waitForLoad !== false) // для ресурсов, которые могут "подвесить" вызов скриптов
	...
	res.onload = function(e)
	{
		...
		if(typeof callback == 'function'){
			callback(function(){
				if(formResAfterLoad[src] != undefined) formResAfterLoad[src].forEach(function(f){ f() });
			});
		}else{
			if(formResAfterLoad[src] != undefined) formResAfterLoad[src].forEach(function(f){ f() });
		}
...
$(function(){
	...
	setTimeout(function(){ checkHeight() }, 0);
	...
	if(window.isMobile || window.isMobileBig){
		...
		setTimeout(function(){ checkOrientation() }, 0);
	...
	$(document).on('change', 'input, textarea, select', function(e) {
		...
		if($e.attr('fakeChange')){ // нужно, например, для multiple-значений select2
		...
		if(type != 'file'){
			...
			var val = $e.val(), code = $e.attr('code');
				...
				if(typeof onSave == 'function'){
					...
					onSave($e, data, function(valid, vdata)
					{
						...
						}else
						{
							...
							if($e.attr('afterSave')){
								wsSendCallback( extend({action: 'save'}, vdata), function(answer){
									window[$e.attr('afterSave')]($e, answer);
								}, function(answer){
									if($e.attr('afterSaveError')) window[$e.attr('afterSaveError')]($e, answer);
								});
							}else{
								wsSendCallback( extend({action: 'save'}, vdata), function(answer){}, function(answer){
									if($e.attr('afterSaveError')) window[$e.attr('afterSaveError')]($e, answer);
								});
							}
						}
					...
					if($e.attr('afterSave')){
						wsSendCallback( extend({action: 'save'}, data), function(answer){
							window[$e.attr('afterSave')]($e, answer);
						}, function(answer){
							if($e.attr('afterSaveError')) window[$e.attr('afterSaveError')]($e, answer);
						});
	...
	$(document).on('click', '[query]', function(e) {
		...
		if($this.attr('onclick') == undefined && !$this.hasClass('edit-mode')){try{ 
			...
			if($this.attr('onclick') == undefined && $this.hasClass('edit-mode') && $this.attr('query') && $('body').hasClass('editMode')){
	...
	if(!window.DISABLE_POPSTATE_CUSTOM_EVENT) window.onpopstate = function(e){
...
locationQuery = function(query){
	...
	if(query.form){
		...
		function loadJS_2(query){
			...
			loadRes('/forms/cache/'+query.form+'_func'+window.userCustomSfx+'.js', false, function(){
				...
				if(query.container){
					...
					wsSend( {action: 'link', query: query, history: $parent.attr('history') }, function(cb){
						...
						$parent
							.addClass('form-block')
							.attr('form', query.form)
							.attr('lastQuery', JSON.stringify(query));
		...
		if(window.debugCSS){
			...
			loadRes('/forms/cache/'+query.form+'_style'+window.userCustomSfx+'.css', true, function(){ loadJS_2(query) });
		}
	}else{
...
elementToHtml = function(o, $parent, code, doAfterLoad, realParent){
	...
	if(o.front) Object.keys(o.front).map(function(key){ o['front-'+key] = o.front[key] });
	
	if(o.front && o.front.onLoad) window[o.front.onLoad](o);
	if(o.front && o.front.onLoadWT) setTimeout(function(){ window[o.front.onLoadWT](o) }, 0);
		...
		if(oo && oo !== 'null' && oo !== 'undefined'){
			...
			if(oo.type){
				...
				if(oo[0] && typeof oo[0] == 'string'){
					...
					oo[0] = oo[0].replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#039;');

					if(oo[1] == undefined) oo[1] = {};
					
					if(oo[1].data){ // оптимизация передачи data-параметров
						Object.keys(oo[1].data).forEach(_=>{ oo[1]['data-'+_] = oo[1].data[_] });
						delete oo[1].data;
					}
					
					// !!! тут проставляется много лишних идентификаторов (например, во внутрь el)
					if(oo[1].code && oo[1].id == undefined){ // для control-el лучше везде проставлять id = false
						oo[1].id = '_'+oo[1].code;
					...
					if(oo[1].complex){
						...
						if(oo[1].complex == 'block'){
							...
							if(oo[1].front && oo[1].front.onLoad) doAfterLoad.push({f: oo[1].front.onLoad, p: realParent, event: 'onLoad'});
						...
						}else{ // тут создание болванки под item
							...
							if(oo[1].li && realParent.attr('onLastItem')) $newparent.attr('onLastItem', realParent.attr('onLastItem'));
						}
					}else{
						...
						if(oo[0] == 'script' || oo[0] == 'style'){
							...
							if(code){ // complex-item
								...
								}catch(e){ console.log(e, oo); $newparent = $('<div />'); }
...
$( document ).ready(function(){
	...
	new MutationObserver(function(mutationsList, observer) {
		for(const mutation of mutationsList) {
			if (mutation.type === 'childList') {
				mutation.removedNodes.forEach(_=>{
					if(window.afterElementMounted[_.id]){
						if(!window.afterElementMounted[_.id].ready) window.afterElementMounted[_.id].f();
					}
				})
			}
	...
	}).observe(document.querySelector('body'), { attributes: true, childList: true, subtree: true, attributeOldValue: true });
});
...
function pingPong(){
	if(!window.ws){
		setTimeout(function(){ pingPong() }, 1000);
		return;
	}
	window.ws.send('ping');
	setTimeout(function(){ pingPong() }, 30000);
}
...
function initWS() {
	...
	window.ws.onopen = function () {
		...
		if(window.wsReconnect){
			...
			var userData = location.href.split('#')[0].split('?')[1], userDataObj = {};
			if(userData){
			...
			if(sessionStorage['xaoc_'+PROJECT+'_session'] == undefined) sessionStorage.setItem('xaoc_'+PROJECT+'_session', '');
			if(userDataObj.session != undefined) sessionStorage.setItem('xaoc_'+PROJECT+'_session', userDataObj.session);
			
			if(localStorage['xaoc_'+PROJECT+'_cur_enter'] != undefined) localStorage.setItem('xaoc_'+PROJECT+'_last_enter', localStorage['xaoc_'+PROJECT+'_cur_enter']);
			if(localStorage['xaoc_'+PROJECT+'_last_enter'] == undefined) localStorage.setItem('xaoc_'+PROJECT+'_last_enter', Date.now());
	...
	window.ws.onmessage = function (msg) {
		...
		if(data.answer){
			...
			if(data.answer.err){
				...
				if(data.answer.badForm && window.setTutorialComplete){ // принудительно закончить обучение
				...
				if(data.req.session != undefined){
					...
					if(data.req.session != data.answer.session) sessionStorage.setItem('xaoc_'+PROJECT+'_session', data.answer.session);

					window.userRole = data.answer.role;
					window.userCustomSfx = (window.userRole ? '_'+window.userRole : '') + (data.answer.theme ? '_'+data.answer.theme : '');
					...
					if(!data.answer.reload)
					{
						...
						if(data.answer.form){ // в частности, для формы логина
							...
							}catch(e){ console.warn(e, "location.hash=", JSON.stringify(location)) }
						}
						
						function loadJS_3(query, callback){
							loadRes('/forms/cache/'+query.form+'_func'+window.userCustomSfx+'.js', false, callback, {gotoOnError: '/', importance: 'high'});
						}
						...
						wsSend( {action: 'form', loopError: data.req.loopError, query: query }, function(callback){
							if(window.debugCSS){
								...
								loadRes('/forms/cache/'+query.form+'_style'+window.userCustomSfx+'.css', true, function(){ loadJS_3(query, callback) });
							}
						});
					}
				}
				if(data.req.query && data.req.query.form && data.answer.load === true){
					...
					if(data.req.query.h === true){ // кнопки вперед/назад
						...
						if(data.req.history !== false && data.req.history !== "false"){
							...
							if(data.req.query.code){
								...
								var query = JSON.parse(history.state?JSON.stringify(history.state):decodeURIComponent(location.hash.split('#')[1]||'{}'));
					...
					function loadJS_1(d){
						loadRes('/forms/cache/'+d.req.query.form+'_script'+window.userCustomSfx+'.js', false, function(){
							if(data.req.history !== false && data.req.history !== "false"){
					...
					if(window.debugCSS)
					{
						...
						if(query && query.subform && query.subform.container && $('#'+query.subform.container).attr('staticForceLoad'))
							src+ '?v='+Date.now();
						loadRes(src, true, function(){ loadJS_1(data) });
					}else{
						loadJS_1(data);
					}
				}
				...
				if(data.answer.load){
					...
					if(code && typeof window.waitForLoadElementByCode[code] == 'function' && !window.waitForLoadElementByCode[code].ready){
						...
						if(data.tpls[id]){
							...
							if($tmp.attr('onLoadElement')){
								if(Object.keys(waitForCodes).length){
									window.doAfterLoadElement[id].doAfterLoad.push({f: $tmp.attr('onLoadElement'), p: $realParent, code: id});
								}else{ // нет вложенных элементов - выполняем сразу
							...
							if($realParent.attr('onItemLoad') && $realParent.attr('code') != id){
								if(Object.keys(waitForCodes).length){
									window.doAfterLoadElement[id].doAfterLoad.push({f: $realParent.attr('onItemLoad'), p: $realParent, code: id});
								}else{ // нет вложенных элементов - выполняем сразу
							...
							if($tmp.attr('onLastItem') && $realParent.attr('onLastItem') && $realParent.attr('code') != id){
								if(Object.keys(waitForCodes).length){
									window.doAfterLoadElement[id].doAfterLoad.push({f: $realParent.attr('onLastItem'), p: $realParent, code: id});
								}else{ // нет вложенных элементов - выполняем сразу
								...
								for(var code in waitForCodes){
									...
									if( window.waitForLoadElementByCode[id] && window.waitForLoadElementByCode[id].w ){
										window.waitForLoadElementByCode[id].w.forEach(function(w){
											if(window.doAfterLoadElement[w] && window.doAfterLoadElement[w].waitForCodes) window.doAfterLoadElement[w].waitForCodes[code] = true;
										})
									}
									...
									window.waitForLoadElementByCode[cCode] = {
										w: [ id ].concat(
											window.waitForLoadElementByCode[id] ? window.waitForLoadElementByCode[id].w : []
										),
									};
...
function reloadForm(e, filter, callback){try{
	var $e = $( e );
...
function myEmpty($e, callback){ // не все очищает, пока не убраны мемлинки (можно этой функцией отловить)
	var $l = $e.find('> *');
	if($l.length){
		$l.each(function(i, e){
			myEmpty($(e), function(){
